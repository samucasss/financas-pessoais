<template>
  <div>
    ...
    <b-table
      ...
      striped
      hover
      :fields="eventoGrid.fields"
      :items="eventoSortList"
    >
...
export default {
  ...
  methods: {
    ...
    async findAllEventos() {
      try {
        const result = await this.$axios.get("/api/eventos");
        this.eventoManager.modelList = result.data.map(json => new Evento(json));
      } catch (e) {
    ...
    async remove(evento) {
      try {
        ...
        await this.$axios.delete("/api/eventos/" + evento.id);
